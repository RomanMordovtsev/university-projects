...
book_stats = ratings.groupby('item_id').agg(
...
merged_data = merged_data.merge(book_stats, on='item_id', how='left')
merged_data = merged_data.merge(user_stats, on='user_id', how='left')

# Векторизация названий книг с использованием разреженных матриц
vectorizer = TfidfVectorizer(max_features=100)  # Уменьшение числа признаков
vectorized_titles = vectorizer.fit_transform(merged_data['title'].fillna(''))  # Оставляем разреженную матрицу

# Преобразование категориальных признаков
categorical_features = merged_data[['author', 'publisher']].fillna('')
one_hot_encoder = OneHotEncoder(sparse_output=True, handle_unknown='ignore')  # Используем разреженную матрицу
one_hot_encoded = one_hot_encoder.fit_transform(categorical_features)

# Подготовка итогового набора признаков
numeric_features = merged_data[['year', 'avg_rating', 'num_ratings', 'avg_user_rating', 'num_user_ratings']].fillna(0).values
numeric_features = StandardScaler().fit_transform(numeric_features)

# Объединение признаков с использованием разреженных матриц
features = hstack([one_hot_encoded, vectorized_titles, numeric_features])
...
with open('linreg_improved.pkl', 'wb') as f:
    pickle.dump(sgd, f)
...
# Преобразуем features в формат csr_matrix для поддержки индексации
...
for item_id in zero_rated_books['item_id']:
    ...
    if svd_pred >= 8:  # Фильтруем книги с прогнозируемым рейтингом >= 8
        # Найти индекс строки в merged_data
        item_indices = merged_data.index[merged_data['item_id'] == item_id].tolist()

        if item_indices:  # Если индекс найден
            # Выбрать соответствующую строку из разреженной матрицы
            book_features = features[item_indices[0]]
            book_features_array = book_features.toarray()  # Преобразовать в плотный массив

            # Прогноз на основе линейной регрессии
            linreg_pred = sgd.predict(book_features_array)[0]
...
files.download('linreg_improved.pkl')  # Загрузка модели линейной регрессии